...
trait LowPriorityNestedGeneric {
  ...
  implicit def notGeneric[In](
      implicit ev: NotLabelledGeneric[In]): Aux[In, OT[In, In]] = {
...
object NestedGeneric extends LowPriorityNestedGeneric {
  ...
  implicit def hnil[In](
      implicit ev: LabelledGeneric.Aux[In, HNil]): Aux[In, OT[In, HNil]] =
    new NestedGeneric[In] {
      type Out = OT[In, HNil]
  ...
  implicit def hcons[In, InLG <: HList, Result <: HList](
      implicit lgen: LabelledGeneric.Aux[In, InLG],
      nestedFolder: Lazy[Mapper.Aux[nestTransform.type, InLG, Result]],
      unnestFolder: Lazy[Mapper.Aux[unnestTransform.type, Result, InLG]])
    : Aux[In, OT[In, Result]] =
    new NestedGeneric[In] {
...
object nestTransform extends Poly1 {
  implicit def default[Aggr <: HList, FieldKey <: Symbol, NextType, NextEntryNG](
      implicit
      nestedGeneric: Lazy[
        NestedGeneric.Aux[NextType, OT[NextType, NextEntryNG]]])
    : Case.Aux[FieldType[FieldKey, NextType],
               OT[NextType, FieldType[FieldKey, NextEntryNG]]] =
...
object unnestTransform extends Poly1 {
  implicit def default[Orig, Aggr <: HList, NextEntryNG, FieldKey <: Symbol](
      implicit
      nestedGeneric: Lazy[NestedGeneric.Aux[Orig, OT[Orig, NextEntryNG]]])
    : Case.Aux[OT[Orig, FieldType[FieldKey, NextEntryNG]],
               FieldType[FieldKey, Orig]] =
