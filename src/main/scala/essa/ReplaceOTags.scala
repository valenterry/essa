...
trait ReplaceOTags[OrigType, NestedGeneric, Target]
    extends DepFn1[OT[OrigType, NestedGeneric]]
    with Serializable
...
trait LowPriorityReplaceOTypes {
  ...
  implicit def forNonGeneric[OrigAndTarget: NotLabelledGeneric: NotGeneric]
    : Aux[OrigAndTarget,
          OrigAndTarget,
          OrigAndTarget,
          OT[OrigAndTarget, OrigAndTarget]] = {
...
object ReplaceOTags extends LowPriorityReplaceOTypes {
  def apply[OrigType, NestedGeneric, Target](
      implicit ev: ReplaceOTags[OrigType, NestedGeneric, Target])
    : Aux[OrigType, NestedGeneric, Target, ev.Out] = ev
  ...
  implicit def nonEmptyRecord[Target,
                              TargetLG <: HList,
                              NestedGeneric <: HList,
                              Zipped <: HList,
                              Orig,
      ...
      nestedFolder: Lazy[Mapper.Aux[changeNGAtoNGB.type, Zipped, Result]])
    : Aux[Orig, NestedGeneric, Target, OT[Target, Result]] =
...
object changeNGAtoNGB extends Poly1 {
  implicit def default[Aggr <: HList, A, AAndBKey <: Symbol, ANG, BNG, B](
      implicit replacer: Lazy[ReplaceOTags.Aux[A, ANG, B, OT[B, BNG]]])
    : Case.Aux[(OT[A, FieldType[AAndBKey, ANG]], FieldType[AAndBKey, B]),
               OT[B, FieldType[AAndBKey, BNG]]] =
